...
export default function BudgetList() {
  ...
  return (
    <Layout>
      ...
      {budgets.length === 0 ? (
        <p className="text-muted-foreground text-sm mt-4">No budgets found. Add one above.</p>
      ) : (
        <ul className="space-y-3 mt-4">
          {budgets.map((b) => (
            <li
              key={b.id}
              className="bg-white dark:bg-muted text-foreground shadow p-4 rounded flex justify-between items-center"
            >
              <div>
