  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <header className="sticky top-0 z-10 h-16 bg-card text-card-foreground shadow-sm flex items-center justify-between px-6">
        ...
        <nav className="flex items-center gap-4">
          ...
          <button
            ...
            className="px-4 py-2 rounded-md text-sm font-medium bg-primary text-primary-foreground hover:bg-primary/90 transition-colors inline-flex items-center gap-2"
            disabled={loadingGetStarted}
          >
            {loadingGetStarted ? <LoadingSpinner /> : "Get Started"}
          </button>
        </nav>
      </header>
      ...
      >
        ...
        >
          {loadingStart ? <LoadingSpinner /> : <>Start Tracking <ArrowRight size={20} /></>}
        </button>
      </motion.section>
