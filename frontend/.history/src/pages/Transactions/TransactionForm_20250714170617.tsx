...
import { DatePicker } from "@/components/DatePicker"; // ✅ Using new DatePicker
...
export default function TransactionForm({ onSubmit, initial, onCancel }: Props) {
  ...
  useEffect(() => {
    ...
    if (initial) {
      setForm({
        ...
        date: new Date(initial.date), // convert to Date
      });
    }
  }, [initial]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const formatted: Omit<Transaction, "id"> = {
      ...form,
      date: form.date.toISOString().slice(0, 10), // string YYYY-MM-DD
    };
  ...
  return (
    <form
      ...
      className="space-y-5 p-4 rounded-lg bg-card text-card-foreground border shadow"
    >
      {/* Category */}
