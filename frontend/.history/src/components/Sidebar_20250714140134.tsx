...
export default function Sidebar({ isSidebarOpen, setIsSidebarOpen }: SidebarProps) {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed md:relative top-0 left-0 w-64 h-screen transform transition-transform z-40 bg-gray-800 text-white md:translate-x-0 ${
      ...
      >
        ...
        {/* Nav Items */}
        <nav className="flex-1 overflow-y-auto pt-2 text-white">
          {[
            ...
            <Link
              ...
              to={to}
              onClick={() => setIsSidebarOpen(false)}
              className={`block px-6 py-3 transition-colors hover:bg-gray-700 ${
          ...
          <button
            ...
            className={`w-full text-left px-6 py-3 transition-colors hover:bg-gray-700 ${
              location.pathname.startsWith("/insights") ? "bg-gray-700 font-semibold" : ""
            }`}
          >
            Insights
          </button>
          ...
          {/* Insights Sub-links */}
          {insightsOpen && (
            <div className="pl-8 text-sm space-y-1 text-white">
              {[
