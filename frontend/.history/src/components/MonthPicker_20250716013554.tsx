...
export function MonthPicker({ selected, onChange }: MonthPickerProps) {
  const [viewMonth, setViewMonth] = useState<Date>(selected);
      ...
      >
        <Calendar
          ...
          onSelect={(date) => {
            ...
            setViewMonth(normalizedDate);   // Sync calendar header
            onChange(normalizedDate);       // Send up to parent
          }}
          ...
          classNames={{
            table: "hidden", // hide day grid if you're only showing month/year
          }}
        />
      </PopoverContent>
    </Popover>
