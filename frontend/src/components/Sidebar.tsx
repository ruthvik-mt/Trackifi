...
export default function Sidebar({ isSidebarOpen, setIsSidebarOpen }: SidebarProps) {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed md:relative top-0 left-0 w-64 h-full md:h-screen transform transition-transform z-40 bg-card text-card-foreground border-r md:translate-x-0 ${
          isSidebarOpen ? "translate-x-0" : "-translate-x-full"
        }`}
      >
       <div className="flex items-center justify-between px-6 py-4">
  <div className="flex items-center space-x-2">
    {/* Logo container with stacked dark/light versions */}
    <div className="relative h-6 w-6">
      {/* Light logo */}
      <img
        src="/logo-light.png"
        alt="Logo Light"
        className="absolute h-6 w-6 object-contain dark:hidden"
      />
      {/* Dark logo */}
      <img
        src="/logo-dark.png"
        alt="Logo Dark"
        className="absolute h-6 w-6 object-contain hidden dark:block"
      />
    </div>

    <span className="text-xl font-bold">Trackifi</span>
  </div>
  <button className="md:hidden" onClick={() => setIsSidebarOpen(false)}>
    <X className="w-6 h-6" />
  </button>
